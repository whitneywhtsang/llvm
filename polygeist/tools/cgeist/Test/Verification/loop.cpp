...
// CHECK-NEXT:       %[[VAL_4:.*]] = memref.alloca() : memref<25x!polygeist.struct<(i32, f64)>>
// CHECK-NEXT:       %[[VAL_K:.*]] = memref.get_global @_ZL1S : memref<i16>
// CHECK-NEXT:       %alloca_0 = memref.alloca() : memref<1xindex>
// CHECK-NEXT:       %reshape = memref.reshape %[[VAL_K]](%alloca_0) : (memref<i16>, memref<1xindex>) -> memref<1xi16>
...
// CHECK-NEXT:         %alloca_1 = memref.alloca() : memref<1xindex>
// CHECK-NEXT:         %reshape_2 = memref.reshape %[[VAL_K]](%alloca_1) : (memref<i16>, memref<1xindex>) -> memref<1xi16>
// CHECK-NEXT:         %[[LOAD:.*]] = affine.load %reshape_2[0] : memref<1xi16>
// CHECK-NEXT:         %[[EXTSI:.*]] = arith.extsi %[[LOAD]] : i16 to i32
// CHECK-NEXT:         %[[ADDI:.*]] = arith.addi %[[VAL_15]], %[[EXTSI]] : i32
// CHECK-NEXT:         %[[VAL_17:.*]] = "polygeist.subindex"(%[[VAL_4]], %[[VAL_14]]) : (memref<25x!polygeist.struct<(i32, f64)>>, index) -> memref<?x!polygeist.struct<(i32, f64)>>
// CHECK-NEXT:         %[[VAL_18:.*]] = "polygeist.subindex"(%[[VAL_17]], %[[VAL_3]]) : (memref<?x!polygeist.struct<(i32, f64)>>, index) -> memref<?xi32>
// CHECK-NEXT:         affine.store %[[ADDI]], %[[VAL_18]][0] : memref<?xi32>
