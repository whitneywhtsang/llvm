...
// CHECK:     func.func @byval(%arg0: memref<?x!polygeist.struct<(i32, i32)>>, %arg1: i32) -> !polygeist.struct<(i32, i32)> attributes {llvm.linkage = #llvm.linkage<external>} {
// CHECK-NEXT:    %0 = affine.load %arg0[0] : memref<?x!polygeist.struct<(i32, i32)>>
// CHECK-NEXT:    return %0 : !polygeist.struct<(i32, i32)>
// CHECK-NEXT:  }
...
// CHECK-NEXT:    %2 = call @byval0(%cast_1, %c2_i32) : (memref<?x!polygeist.struct<(i32, i32)>>, i32) -> !polygeist.struct<(i32, i32)>
// CHECK-NEXT:    affine.store %2, %alloca[0] : memref<1x!polygeist.struct<(i32, i32)>> 
// CHECK-NEXT:    %3 = "polygeist.subindex"(%cast, %c0) : (memref<?x!polygeist.struct<(i32, i32)>>, index) -> memref<?xi32> 
// CHECK-NEXT:    %4 = affine.load %3[0] : memref<?xi32> 
